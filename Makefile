...
VERILOG_FILES=$(wildcard *.sv CPU/*.sv)

RM         = rm -rf
COPY       = cp -a
PATH_SEP   = /

# Yosys include directory
...
%.ys: $(VERILOG_FILES)
	...
	$(if $(DO_SIMULATION), \
		$(file >>$@,write_cxxrtl $(TOP_MODULE).cpp) \
		, \
...
$(PROJ).cpp: DO_SIMULATION=yes
$(PROJ).cpp: $(PROJ).ys
	yosys -s "$<"

%.vcd: $(TOP_SIMULATION_MODULE).cpp $(PROJ).cpp
	g++ -g -std=c++14 -I $(YOSYS_INCLUDE) $(TOP_SIMULATION_MODULE).cpp -o $(TOP_SIMULATION_MODULE)
	./$(TOP_SIMULATION_MODULE)
...
clean:
	$(RM) -f $(PROJ).svf $(PROJ).bit $(PROJ)_out.config $(PROJ).json $(PROJ).dfu $(PROJ).vcd $(PROJ).cpp $(PROJ).ys $(TOP_SIMULATION_MODULE)
